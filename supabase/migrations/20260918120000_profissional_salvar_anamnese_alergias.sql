-- RELATÓRIO 20260918_0001 (continuação 2) — achado na auditoria final:
-- `profissional_salvar_anamnese` cobria condições de saúde (Bloco 8) mas
-- não Alergias, apesar de "Alergias: Preencher a lista com o padrão
-- clínico" ser parte explícita da tarefa. Mesmo padrão do loop de
-- `condicoes_saude` já existente, só que gravando em `anamneses_alergias`.
...
begin
  ...
  -- Bloco 5 (Alimentação) — alergias, mesma tabela N:N que o app Flutter
  -- já usa (`anamneses_alergias`, `20260811240000`).
...
comment on function public.profissional_salvar_anamnese(uuid, jsonb) is
  'RELATÓRIO 20260917_0001 (item 2) + RELATÓRIO 20260918_0001 (compliance total, Blocos 1-16, incl. alergias): profissional com vínculo ATIVO preenche uma anamnese nova pro paciente, cobrindo os mesmos campos que o self-service do App grava. SEM trava de carência.';
